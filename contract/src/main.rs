...
use crate::{
    constants::{
        ...
        COLLECTION_ROYALTY, COLLECTION_SYMBOL, CONTRACT_NAME_PREFIX, CONTRACT_VERSION_PREFIX,
        CONTRACT_WHITELIST, ENTRY_POINT_APPROVE, ENTRY_POINT_BALANCE_OF, ENTRY_POINT_BURN,
        ENTRY_POINT_GET_APPROVED, ENTRY_POINT_INIT, ENTRY_POINT_METADATA, ENTRY_POINT_MIGRATE,
        ...
        PAGE_LIMIT, PAGE_TABLE, RECEIPT_NAME, REPORTING_MODE, ROYALTIES, TOKEN_COUNTS,
        TOKEN_ISSUERS, TOKEN_OWNERS, TOTAL_TOKEN_SUPPLY, UNMATCHED_HASH_COUNT, WHITELIST_MODE,
    },
    error::NFTCoreError,
...
pub extern "C" fn init() {
    ...
    runtime::put_key(
        COLLECTION_SYMBOL,
        storage::new_uref(collection_symbol.clone()).into(),
    );
    runtime::put_key(
        TOTAL_TOKEN_SUPPLY,
    ...
    runtime::put_key(JSON_SCHEMA, storage::new_uref(json_schema.clone()).into());
    ...
    runtime::put_key(
        CONTRACT_WHITELIST,
        storage::new_uref(contract_whitelist.clone()).into(),
    );
    ...
    storage::new_dictionary(PAGE_TABLE)
        .unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
    storage::new_dictionary(ROYALTIES)
        .unwrap_or_revert_with(NFTCoreError::FailedToCreateDictionary);
    ...
    emit_cep78(&CEP78Event::Init {
        collection_name,
        collection_symbol,
        total_token_supply,
        allow_minting,
        minting_mode: minting_mode as u8,
        ownership_mode: ownership_mode as u8,
        nft_kind: nft_kind as u8,
        holder_mode: holder_mode as u8,
        whitelist_mode: whitelist_mode as u8,
        contract_whitelist,
        json_schema,
        nft_metadata_kind: nft_metadata_kind as u8,
        identifier_mode: identifier_mode as u8,
        metadata_mutability: metadata_mutability as u8,
        burn_mode: burn_mode as u8,
        reporting_mode: reporting_mode as u8,
    });
}
...
pub extern "C" fn set_variables() {
    ...
    emit_cep78(&events::CEP78Event::SetVariables {
        allow_minting: utils::get_stored_value_with_user_errors::<bool>(
            ALLOW_MINTING,
            NFTCoreError::MissingAllowMinting,
            NFTCoreError::InvalidAllowMinting,
        ),
        contract_whitelist: utils::get_stored_value_with_user_errors::<Vec<ContractHash>>(
            CONTRACT_WHITELIST,
            NFTCoreError::MissingWhitelistMode,
            NFTCoreError::InvalidWhitelistMode,
        ),
...
pub extern "C" fn transfer() {
    ...
    emit_cep78(&CEP78Event::Transfer {
        token_id: token_identifier.clone(),
        to: target_owner_key,
        from: token_owner_key,
        from_balance: updated_from_account_balance,
        to_balance: updated_to_account_balance,
    });
...
pub extern "C" fn set_token_metadata() {
    ...
    emit_cep78(&CEP78Event::UpdateMetadata {
        token_id: token_identifier,
...
fn generate_entry_points() -> EntryPoints {
    ...
    let set_token_royalty = EntryPoint::new(
        "set_token_royalty",
        vec![
            Parameter::new("royalty", CLType::U64),
    ...
    let royalty_info = EntryPoint::new(
        "royalty_info",
        vec![
            Parameter::new("royalty", CLType::U64),
            Parameter::new("recipient", CLType::Key),
    ...
    let set_collection_royalty = EntryPoint::new(
    ...
    entry_points.add_entry_point(set_token_royalty);
    entry_points.add_entry_point(royalty_info);
    entry_points.add_entry_point(set_collection_royalty);
...
#[no_mangle]
pub extern "C" fn set_token_royalty() {
    let installer = utils::get_account_hash(
        INSTALLER,
        NFTCoreError::MissingInstaller,
        NFTCoreError::InvalidInstaller,
    ...
    // Only the installing account can set the royalty.
    if installer != runtime::get_caller() {
        runtime::revert(NFTCoreError::InvalidAccount);
    }
    ...
    let token_identifier = utils::get_token_identifier_from_runtime_args(&identifier_mode);
    let royalty: u64 = runtime::get_named_arg("royalty");
    let recipient: Key = runtime::get_named_arg("recipient");

    let royalty: Royalty = (royalty, recipient);
...
pub extern "C" fn set_collection_royalty() {
    ...
    runtime::put_key(COLLECTION_ROYALTY, storage::new_uref(royalty).into());
}

#[no_mangle]
pub extern "C" fn royalty_info() {
    let sale_price: U256 = runtime::get_named_arg("sale_price");
    ...
    let token_royalty_info: Option<(u64, Key)> = utils::get_dictionary_value_from_key(
        ROYALTIES,
        &token_identifier.get_dictionary_item_key(),
    );

    // if the royalty is not set for the token, check if it is set for the collection
    let royalty_info = if let Some(info) = token_royalty_info {
        info
    } else {
        utils::get_stored_value_with_user_errors::<Royalty>(
            COLLECTION_ROYALTY,
            NFTCoreError::MissingAllowMinting,
            NFTCoreError::InvalidAllowMinting,
        )
